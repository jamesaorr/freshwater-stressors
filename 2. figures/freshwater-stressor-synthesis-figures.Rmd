...
world <- world %>%
  ...
  dplyr::select(name_long, experiments)
```
...
identity_data <- experiments %>%
  dplyr::select(contains("Identity")) %>%
...
## Figure 4 - Stressor Classes

- *Stressor natures over time*

```{r}
class_data <- experiments %>%
  dplyr::select(c(Year, contains("Class"))) %>%
